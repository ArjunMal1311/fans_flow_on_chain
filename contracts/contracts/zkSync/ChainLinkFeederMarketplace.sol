...
contract ZkNFTMarketplace is ReentrancyGuard, Ownable {
    ...
    function buyNFT(uint256 listingId) public payable nonReentrant {
        ...
        (address royaltyReceiver, uint256 royaltyAmount) = nftContract.royaltyInfo(listingId, listing.price);
        if (royaltyAmount > 0) {
        ...
        nftContract.safeTransferFrom(listing.seller, msg.sender, listingId, 1, "");

        listing.isListed = false;

        emit NFTSold(listing.seller, msg.sender, listingId, listing.price);
    }
